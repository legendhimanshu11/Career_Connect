...
const FresherJobs = () => {
  ...
  const jobsPerPage = 6;
  const fresherJobs = allJobs.filter((job) => job.tags.includes('Fresher'));

  useEffect(() => {
    let jobs = fresherJobs.filter(
      (job) =>
    ...
    if (sortOption === 'title') {
      ...
      jobs.sort((a, b) => a.company.localeCompare(b.company)); // Fixed
    }
      ...
      <div className="container mx-auto px-4 xl:px-20 py-10">
        ...
        >
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-800">
            Explore <span className="text-blue-600">Fresher Jobs</span>
          </h1>
          <p className="text-gray-500 mt-2">Opportunities to kickstart your career</p>
        </motion.div>
        ...
        >
          {currentJobs.length > 0 ? (
            currentJobs.map((job, index) => (
              ...
              >
                <JobCard job={job} highlightTags={['Fresher']} />
              </motion.div>
            ))
          ) : (
