...
const StartupJobs = () => {
  ...
  const startupJobs = allJobs.filter((job) => job.tags.includes('Startup'));

  useEffect(() => {
    let jobs = startupJobs.filter(
      (job) =>
    ...
    setSlideIndex(0); // Reset to first slide on filter change
  ...
  useEffect(() => {
    const interval = setInterval(nextSlide, 5000); // every 5 seconds
      ...
      <div className="container mx-auto px-4 xl:px-20 py-10">
        ...
        >
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-800">
            Discover <span className="text-blue-600">Startup Jobs</span>
          </h1>
          <p className="text-gray-500 mt-2">Exciting roles in fast-growing startups</p>
        </motion.div>
        ...
        >
          {currentJobs.map((job, index) => (
            <JobCard key={index} job={job} highlightTags={['Startup']} />
          ))}
        </motion.div>
        ...
        {filteredJobs.length === 0 && (
          <p className="text-center text-gray-500 mt-10">No startup jobs found.</p>
        )}
      </div>
      <Footer />
    </div>
