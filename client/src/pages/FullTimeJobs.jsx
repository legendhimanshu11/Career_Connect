...
const FullTimeJobs = () => {
  ...
  const jobsPerPage = 6;
  const fullTimeJobs = allJobs.filter((job) => job.tags.includes('Full-time'));

  useEffect(() => {
    let jobs = fullTimeJobs.filter(
      (job) =>
    ...
    if (sortOption === 'title') {
      ...
      jobs.sort((a, b) => a.company.localeCompare(b.company)); // Fix for plain string
    }
      ...
      <div className="container mx-auto px-4 xl:px-20 py-10">
        ...
        >
          <h1 className="text-3xl sm:text-4xl font-bold text-gray-800">
            Explore <span className="text-blue-600">Full-time Jobs</span>
          </h1>
          <p className="text-gray-500 mt-2">Apply for top full-time roles now</p>
        </motion.div>
        ...
        >
          {currentJobs.length > 0 ? (
            currentJobs.map((job, index) => (
              ...
              >
                <JobCard job={job} highlightTags={['Full-time']} />
              </motion.div>
            ))
          ) : (
            <p className="text-center text-gray-500 col-span-full">No full-time jobs found.</p>
          )}
        </motion.div>
